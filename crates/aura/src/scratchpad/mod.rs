...
// The serializable scratchpad config knobs live in `aura-config`; re-export
// them here so `crate::scratchpad::{ScratchpadConfig, ScratchpadToolEntry}`
// keeps resolving for the runtime machinery in this module.
pub use aura_config::{ScratchpadConfig, ScratchpadToolEntry};

/// Runtime configuration for scratchpad tools, passed via the agent runtime
...
mod tests {
    ...
    // Note: `ScratchpadConfig` / `ScratchpadToolEntry` deserialization tests
    // live in `aura-config` (where the types are now defined). This module
    // only tests the runtime `scratchpad_tool_map` resolution below.
